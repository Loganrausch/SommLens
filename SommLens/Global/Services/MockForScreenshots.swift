//
//  MockForScreenshots.swift
//  SommLens
//
//  Created by Logan Rausch on 1/9/26.
//
...
struct DebugLauncherView: View {
    ...
    // Create these once so they stay stable across presentations
    private let ctx = PersistenceController.shared.container.viewContext
    @StateObject private var openAIManager = OpenAIManager()

    var body: some View {
        VStack {
            Button("Launch ScanResultView") {
                launch = true
            }
        }
        .fullScreenCover(isPresented: $launch) {
            ScanResultView(
                bottle: BottleScan(context: ctx),
                capturedImage: UIImage(named: "TESTWINE3") ?? UIImage(systemName: "photo")!,
                wineData: debugWineData(),
                onDismiss: { launch = false },
            ...
            .environmentObject(AuthViewModel()) // required because ScanResultView uses @EnvironmentObject auth
        }
    }

    private func debugWineData() -> WineData {
        let json = """
        {
        ...
        do {
            return try JSONDecoder().decode(WineData.self, from: Data(json.utf8))
        } catch {
            fatalError("❌ Failed to decode WineData: \(error)")
        }
    }
}
#endif
...
struct TestDetailLauncher: View {
    @Environment(\.managedObjectContext) private var ctx
    @FetchRequest(
        entity: BottleScan.entity(),
        sortDescriptors: [],
        animation: .default
    ...
    var body: some View {
        if let mockScan = findFakeScan(named: "Domaine Luneval", from: Array(scans)),
           let json = mockScan.rawJSON,
           let wine = try? JSONDecoder().decode(WineData.self, from: Data(json.utf8)) {

            WineDetailView(
                bottle: mockScan,
                wineData: wine,
                snapshot: UIImage(named: "TESTWINE3"),
                openAIManager: openAIManager,
                ctx: ctx
            )
            .environmentObject(openAIManager)
            .environmentObject(AuthViewModel()) // WineDetailView needs auth env object
        } else {
            DebugFailView("Couldn't find mock scan. Did you seed scans?")
        }
    }
}
#endif
